...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (downloadCvBtn) {
        downloadCvBtn.addEventListener('click', (e) => {
            e.preventDefault();
            window.open('Virendra_Bagul_DevOps_Resume.pdf', '_blank');
        });
    }
    ...
    const counterObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                let current = 0;
                const noPlus = el.hasAttribute('data-noplus');
                const timer = setInterval(() => {
                    current += step;
                    if (current >= target) {
                        el.textContent = noPlus ? target : target + '+';
                        clearInterval(timer);
                    } else {
                        el.textContent = noPlus ? Math.floor(current) : Math.floor(current) + '+';
                    }
                }, 16);
    ...
    if (terminalInput && terminalOutput) {
        ...
        terminalInput.addEventListener('keydown', (e) => {
            if (e.key === 'Enter') {
                ...
                userCommandLine.innerHTML = `<span class="term-prompt">virendra@infrastructure:~$</span> <span class="term-echoed">${escapeHTML(commandText)}</span>`;
        ...
        async function processCommand(cmd) {
            ...
            switch (cmd) {
                case 'help':
                    outputDiv.innerHTML = `
                        ...
                        <div style="margin-left: 15px; margin-top: 4px; display: grid; grid-template-columns: 140px 1fr; gap: 4px; font-family: Courier New, monospace; font-size: 0.85rem;">
                            <div><span class="term-cyan">about</span></div><div>Display Virendra's bio & summary</div>
                ...
                case 'ansible-run':
                    ...
                    printAnsibleLine("changed: [aws-ec2-linux-1] => (item=virendra)   [added SSH key authorized_keys]", "term-yellow");
